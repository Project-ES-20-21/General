...
```
### Buffers
Omdat er veel waarden ,met uitschieters die niet gewenst zijn, worden gemeten op korte tijd (er moet snel gemeten worden), worden circulaire buffers gebruikt zodat het gemiddelde van de buffer kan worden genomen. De buffers worden geïnitialiseerd in de setup via de methode `initBuffers(size)`, met `size` als instelbare variabele. Er worden RSSI waarden gemeten t.o.v drie andere modules, er zijn dus drie andere buffers nodig. Omdat de code telkens naar vier verschillende ESP's geüpload worden, worden er direct vier buffers aangemaakt zodat deze niet elke keer opnieuw aangepast moeten worden.  
```c
    initBuffers(size);
```
```c
...
## BLE Callback
Deze callback zorgt voor de verwerking van de resultaten van de BLE scan. Eerst wordt de naam van de verzender van het ontvangen signaal vergeleken met een aantal strings zodat we zeker zijn van welke andere ESP32 het signaal komt. De RSSI waarde van het signaal wordt in de buffer gestoken en een teller wordt verhoogd. Als de teller gelijk is aan de buffer grootte, wat betekent dat elke waarde vernieuwd is ten opzichte van de vorige keer dat de teller gelijk was aan de buffer grootte, zijn er dus buffer grootte aantal nieuwe RSSI waarden gemeten. Hierdoor kunnen de resultaten uit de buffer verwerkt worden. De teller wordt terug op nul gezet. Welke teller en buffer er verandert wordt, hangt af van welke ESP het signaal komt, als "Afstand_2" iets ontvangt van Afstand_0 dan zullen `teller0` en `buffer0` aangepast worden. Als de `send_to_broker` variabele "true" is, de wachttijd van de vorige overtreding gedaan is en er wordt een overtreding begaan, is het gemiddelde van de buffer dus kleiner dan de opgestelde grenswaarde: dan moet het alarm afgaan. De `send_to_broker` parameter is een variabele die weergeeft of dat de personen zich hebben ontsmet sinds de vorige fout. Indien er een overtreding is gedetecteerd, moet er een melding gestuurd worden naar de ontsmettingsgroep met daarin de id's van de overtreders. Dit gebeurd door `beginPiep` op "true" te zetten. Op het einde van de callback functie wordt de `non blockingPiep()` methode opgeroepen. Als `beginPiep` op "true" staat zal deze functie het alarm laten afgaan.
...
```
## MQTT
De MQTT callback functie wordt in de loop opgeroepen via `client.loop()`. Als er een bericht is op een kanaal waarop gesubscribed is dan zal de callback dit bericht verwerken. Eerst wordt gekeken op welk kanaal het bericht verzonden is. 
